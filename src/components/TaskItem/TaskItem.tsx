...
import { deleteTask, changeDataTask } from '../../api/todo';
...
const TaskItem: FC<TaskItemProps> = ({
  ...
  const handleChangeDataTask = async (
    id: number,
    titleTask: string,
    isDone: boolean,
  ) => {
      ...
      <Flex justify="space-between" className={classes.task}>
        <Flex align="center" gap={'0.5rem'} style={{ width: '100%' }}>
          <Checkbox
            ...
            onClick={() => handleChangeDataTask(id, titleTask, !isDone)}
          />
          {!isEdited ? (
              ...
              rules={[
                {
                  ...
                  min: 2,
                  message:
                    'Нельзя изменить задачу с количеством символов в названии меньше 2-х!',
                },
                {
                  max: 64,
                  message:
                    'Нельзя изменить задачу с количеством символов в названии больше 64-х!',
                },
              ]}
            >
